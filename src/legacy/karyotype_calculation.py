...
segments_df = pd.read_csv(r".\segments_of_all_samples.csv").fillna(np.nan)
...
for cell in cells_set:
    ...
    for chromosome_arm in chromosome_arm_list:
        ...
        if chromosome_arm not in [str(x) + "p" for x in ACROCENTRIC_LIST]:
            segments_curr_cell_arm["SegmentMean"] = segments_curr_cell_arm["SegmentMean"].fillna(0)

        segments_curr_cell_arm["weighted_seg"] = segments_curr_cell_arm["segment_length"] * segments_curr_cell_arm["SegmentMean"]
        s = sum(segments_curr_cell_arm["weighted_seg"].to_list())
        arm_cn = s / segments_curr_cell_arm["segment_length"].sum()
