...
# Script location: repo_root/src/postprocessing/02_PDO_sample_analysis.py
...
metadata = pd.read_csv(os.path.join(root, "data", "metadata_organoids.csv"), header=0)
...
for _, row in metadata.iterrows():
    ...
    matches_table = pd.read_csv(os.path.join(root, "results", "Cohort_data", f"{row['cohort_index']}_{cohort_name}", f"{row['cohort_index']}_{cohort_name}_matches_table.csv"))
...
for _, row in all_sample_matches_reshaped.iterrows():
    ...
    # Load pre-processed CNV data (normalized bins and arm-level)
    # Generated by preprocessing pipeline in results/Cohort_data/
    bins_table = pd.read_csv(os.path.join(root, "results", "Cohort_data", f"{cohort_index}_{cohort_name}", f"{cohort_index}_{cohort_name}_mb_bins_normalized.csv"), index_col=0)
    arms_table = pd.read_csv(os.path.join(root, "results", "Cohort_data", f"{cohort_index}_{cohort_name}", f"{cohort_index}_{cohort_name}_arm_level_normalized.csv"), index_col="Sample_ID", header=0)
