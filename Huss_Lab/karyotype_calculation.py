...
BUILD_FILE = r"..\chr_arm_annotations_modified_xy_hg38.xlsx"
...
for cell in cells_set:
    ...
    for chromosome_arm in chromosome_arm_list:
        ...
        # print(start, end)
        # print(segments_curr_cell.dtypes)
        ...
        # print(segments_curr_cell_arm.head(50).to_string())
...
print(x.to_string())
# save_col = x.columns.to_list()[1:]
# x["Plate_number"] = x["Sample_ID"].apply(lambda x: int(x.split("_")[0]))
# x["Condition"] = x["Sample_ID"].apply(lambda x: x.split("_")[1])
# x["Cell_number"] = x["Sample_ID"].apply(lambda x: int(x.split("_")[2]))
# x = x.loc[:, ["Sample_ID", "Plate_number", "Condition", "Cell_number"] + save_col]
# x = x.sort_values(["Plate_number", "Condition", "Cell_number"]).reset_index(drop=True)
